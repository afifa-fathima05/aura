...
export const addContactSubmission = async (contactData: Omit<ContactSubmission, 'id' | 'createdAt' | 'status'>): Promise<string> => {
  try {
    console.log('🔥 Adding contact submission to Firestore...', contactData.subject)
...
export const getContactSubmissions = async (): Promise<ContactSubmission[]> => {
  try {
    ...
    console.log(`✅ Fetched ${submissions.length} contact submissions from Firestore`)
    return submissions
...
export const addMembershipApplication = async (applicationData: Omit<MembershipApplication, 'id' | 'createdAt' | 'status' | 'membershipId'>): Promise<string> => {
  try {
    console.log('🔥 Adding membership application to Firestore...', applicationData.name)
    ...
    // Save using the pre-created doc ref to avoid extra reads
    await setDoc(newDocRef, dataToSave)
    console.log('✅ Membership application added to Firestore with ID:', newDocRef.id, 'membershipId:', membershipId)
    return newDocRef.id

    
  } catch (error) {
    console.error('❌ Error adding membership application to Firestore:', error)
    throw error
  }
}
...
export const getMembershipApplications = async (): Promise<MembershipApplication[]> => {
  try {
    ...
    console.log(`✅ Fetched ${applications.length} membership applications from Firestore`)
    return applications
